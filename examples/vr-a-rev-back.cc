...
NS_LOG_COMPONENT_DEFINE("VrARevBack");
...
void
ClientRxDelay(Ptr<OutputStreamWrapper> traceFile, const Time& time, const Address& address)
{
    NS_LOG_INFO("Client received a packet with delay of " << time.GetSeconds() << " seconds from " << address);

    *traceFile->GetStream() << Simulator::Now().GetNanoSeconds() << "," << AddressToString(address) <<  "," << time.GetNanoSeconds() << "\n";
}

void
ClientRxRtt(Ptr<OutputStreamWrapper> traceFile, const Time& time, const Address& address)
{
    NS_LOG_INFO("Client received a packet with RTT of " << time.GetSeconds() << " seconds from " << address);
...
void
ClientMainObjectReceived(Ptr<OutputStreamWrapper> traceFile, Ptr<const ThreeGppHttpClient>, Ptr<const Packet> packet)
{
        ...
        header.GetContentType() == ThreeGppHttpHeader::MAIN_OBJECT)
    {
        ...
        *traceFile->GetStream() << Simulator::Now().GetNanoSeconds() << "," << p->GetSize() << "\n";
    }
    else
    {
...
void
ClientEmbeddedObjectReceived(Ptr<OutputStreamWrapper> traceFile, Ptr<const ThreeGppHttpClient>, Ptr<const Packet> packet)
{
    Ptr<Packet> p = packet->Copy();
    ThreeGppHttpHeader header;
    p->RemoveHeader(header);
...
main(int argc, char* argv[])
{
    ...
    LogComponentEnable("VrARevBack", LOG_INFO);
    ...
    // Setting background traffic
    ThreeGppHttpServerHelper serverBackHelper(Ipv4Address::GetAny());
    ApplicationContainer serverBackApps = serverBackHelper.Install(wifiApNode);
    Ptr<ThreeGppHttpServer> httpServer = serverBackApps.Get(0)->GetObject<ThreeGppHttpServer>();
    ...
    ThreeGppHttpClientHelper clientBackHelper(ApInterface.GetAddress(0));
    ApplicationContainer clientBackApps = clientBackHelper.Install(wifiStaNodes);

    // Setup traces
    AsciiTraceHelper ascii;

    Ptr<OutputStreamWrapper> backBytesTrace = ascii.CreateFileStream("backBytesTrace.csv");
    *backBytesTrace->GetStream() << "RxTime_ns,Bytes" << std::endl;

    Ptr<OutputStreamWrapper> backRttTrace = ascii.CreateFileStream("backRttTrace.csv");
    *backRttTrace->GetStream() << "RxTime_ns,To,Rtt_ns" << std::endl;

    Ptr<OutputStreamWrapper> backDelayTrace = ascii.CreateFileStream("backDelayTrace.csv");
    *backDelayTrace->GetStream() << "RxTime_ns,To,Delay_ns" << std::endl;


    for (uint32_t i = 0; i < clientBackApps.GetN (); i++) {
        Ptr<ThreeGppHttpClient> httpClient = clientBackApps.Get(i)->GetObject<ThreeGppHttpClient>();
        ...
        httpClient->TraceConnectWithoutContext("RxMainObject", MakeBoundCallback(&ClientMainObjectReceived,backBytesTrace));
        httpClient->TraceConnectWithoutContext("RxEmbeddedObject",
                                            MakeBoundCallback(&ClientEmbeddedObjectReceived,backBytesTrace));
        httpClient->TraceConnectWithoutContext("Rx", MakeCallback(&ClientRx));
        httpClient->TraceConnectWithoutContext("RxDelay", MakeBoundCallback(&ClientRxDelay,backDelayTrace));
        httpClient->TraceConnectWithoutContext("RxRtt", MakeBoundCallback(&ClientRxRtt,backRttTrace));
    ...
    *fragmentTrace->GetStream()
    ...
    for (uint32_t i = 0; i < nStas; i++)
    {
        ...
        app->SetAttribute("Local", AddressValue(InetSocketAddress(StaInterface.GetAddress(i), 0)));
