...
def compute_qoe(results):
    ...
    if len(bytes_recv) > 0:
        avg_throughput = np.sum(bytes_recv) / results['params']['simulationTime']/ results['params']['ueNumPergNb']
...
if __name__ == '__main__':
    ...
    if args.paramSet == "ueNumPergNb":
        param_combination = OrderedDict({
            "appRate": "50Mbps",
            "frameRate": args.frameRate,
            # "burstGeneratorType": ["model", "fuzzy", "google", "bola", "mpc", "festive"],
            "burstGeneratorType": ["oran-util-udp", "model", "fuzzy"],
            # "ueNumPergNb": list(range(1, 16+1, 5)),
            "ueNumPergNb": list(range(1, 36+1, 5)),
            "simulationTime": 10,
