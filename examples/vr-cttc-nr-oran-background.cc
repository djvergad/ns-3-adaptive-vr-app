...
void
UpdateOnOffRate(Ptr<OnOffApplication> app,
                Ptr<UniformRandomVariable> rateRv,
                uint32_t minRateKbps,
                uint32_t maxRateKbps,
                Time updatePeriod,
                Time stopTime)
{
    if (app == nullptr || Simulator::Now() > stopTime)
    {
        return;
    }

    uint32_t nextRateKbps = rateRv->GetInteger(minRateKbps, maxRateKbps);
    app->SetAttribute("DataRate", DataRateValue(DataRate(nextRateKbps * 1000)));
...
main(int argc, char* argv[])
{
    ...
    uint16_t vrTrafficNodeCount = 2;
    uint16_t backgroundNodeCount = 10;
    ...
    cmd.AddValue("lambdaBe",
                 ...
                 lambdaBe);
    cmd.AddValue("vrTrafficNodeCount",
                 "Number of UE nodes that carry VR traffic",
                 vrTrafficNodeCount);
    cmd.AddValue("backgroundNodeCount",
                 "Number of UE nodes that carry background traffic",
                 backgroundNodeCount);
    ...
    // Ensure there are enough UEs to support both VR and background traffic.
    uint16_t totalUes = ueNumPergNb * gNbNum;
    uint16_t requiredUes = vrTrafficNodeCount + backgroundNodeCount;
    if (totalUes < requiredUes)
    {
    ...
    for (uint32_t j = 0; j < gridScenario.GetUserTerminals().GetN(); ++j)
    {
        ...
        if (j < vrTrafficNodeCount)
        {
            ueLowLatContainer.Add(ue);
        }
        else if (j < vrTrafficNodeCount + backgroundNodeCount)
        {
    ...
    *txFragmentTrace->GetStream()
    ...
    ApplicationContainer backgroundSinkApps;
    ...
    for (uint16_t i = 0; i < backgroundNodeCount; ++i)
    {
        ...
        backgroundOnOff.SetAttribute("OffTime",
        ...
        Ptr<UniformRandomVariable> rateRv =
            CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                              DoubleValue(backgroundMinRateKbps),
                                                              "Max",
                                                              DoubleValue(backgroundMaxRateKbps));

        Simulator::Schedule(backgroundStart,
                            &UpdateOnOffRate,
                            onOff,
                            rateRv,
