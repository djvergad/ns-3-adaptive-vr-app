...
void
SetLinkRate (DataRate dataRate, NetDeviceContainer *devices)
{
  NS_LOG_INFO ("Setting link rate to " << dataRate.GetBitRate () / 1e6 << "Mbps");

  for (uint32_t i = 0; i < devices->GetN (); i++)
    {
      devices->Get (i)->SetAttribute ("DataRate", DataRateValue (dataRate));
    }
}
...
main (int argc, char *argv[])
{
  ...
  pointToPoint.SetDeviceAttribute ("DataRate", StringValue ("38Mbps"));
  ...
  Simulator::Schedule (Seconds (5), &SetLinkRate, DataRate ("5Mbps"), &devices);
  Simulator::Schedule (Seconds (10), &SetLinkRate, DataRate ("38Mbps"), &devices);
