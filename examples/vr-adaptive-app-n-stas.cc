...
main (int argc, char *argv[])
{
  ...
  if (burstGeneratorType == "model" || burstGeneratorType == "adaptive" ||
      burstGeneratorType == "tcp" || burstGeneratorType == "tcp_adaptive")
    {
      ...
      client.SetBurstGenerator ("ns3::VrBurstGenerator", "FrameRate", DoubleValue (frameRate),
                                "TargetDataRate", DataRateValue (DataRate (appRate)), "VrAppName",
  ...
  else if (burstGeneratorType == "trace")
    {
      ...
      filenameSs << GetInputPath () << "contrib/vr-app/model/BurstGeneratorTraces/" << appAbbrev
                 << "_" << dataRateMbps << "mbps_" << uint32_t (frameRate) << "fps.csv";

      NS_LOG_DEBUG ("Trace file generator with filename=" << filenameSs.str ());

      client.SetBurstGenerator ("ns3::TraceFileBurstGenerator", "TraceFile",
  ...
  else if (burstGeneratorType == "deterministic")
    {
      ...
      client.SetBurstGenerator ("ns3::SimpleBurstGenerator", "PeriodRv",
                                StringValue (periodSs.str ()), "BurstSizeRv",
