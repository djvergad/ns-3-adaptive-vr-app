...
double
ToMbpsFromDataRateString(const std::string& rate)
{
    DataRate dr(rate);
    return static_cast<double>(dr.GetBitRate()) / 1e6;
}

std::vector<double>
ParseWeightsCsv(const std::string& csv, uint32_t ueCount)
{
    std::vector<double> parsed;
    for (const auto& token : SplitString(csv, ','))
    {
        if (token.empty())
        {
            continue;
        ...
        {
            // Ignore malformed tokens and keep defaults for safety.
        }
    }
    ...
    std::vector<double> weights(ueCount, 1.0);
    for (uint32_t i = 0; i < ueCount; ++i)
    {
        weights.at(i) = parsed.at(i % parsed.size());
    }
    return weights;
}

std::vector<double>
ComputeWeightedPfAllocation(const std::vector<double>& demandMbps,
                            const std::vector<double>& weights,
                            double ruCapacityMbps,
                            double duCapacityMbps,
                            double duCostPerMbps)
{
    std::vector<double> allocation(demandMbps.size(), 0.0);
    if (demandMbps.empty() || weights.size() != demandMbps.size())
    {
        return allocation;
    }

    const double epsilon = 1e-9;
    const double effectiveDuCost = std::max(duCostPerMbps, epsilon);
    const double effectiveCapacityMbps =
        std::max(0.0, std::min(ruCapacityMbps, duCapacityMbps / effectiveDuCost));
    if (effectiveCapacityMbps <= epsilon)
    {
        return allocation;
    }

    const double totalDemand =
        std::accumulate(demandMbps.begin(), demandMbps.end(), 0.0, [](double acc, double d) {
            return acc + std::max(0.0, d);
        });
    if (totalDemand <= effectiveCapacityMbps)
    {
        for (uint32_t i = 0; i < demandMbps.size(); ++i)
        {
            allocation.at(i) = std::max(0.0, demandMbps.at(i));
        }
        return allocation;
    }

    auto SumAtLambda = [&](double lambda) {
        double sum = 0.0;
        for (uint32_t i = 0; i < demandMbps.size(); ++i)
        {
            sum += std::min(std::max(0.0, demandMbps.at(i)), std::max(0.0, weights.at(i)) * lambda);
        }
        return sum;
    };
    ...
    double low = 0.0;
    double high = 1.0;
    while (SumAtLambda(high) < effectiveCapacityMbps)
    {
        high *= 2.0;
        if (high > 1e8)
        {
    ...
    for (uint32_t k = 0; k < 60; ++k)
    {
        ...
        if (SumAtLambda(mid) < effectiveCapacityMbps)
        {
            low = mid;
        }
        else
    ...
    {
        allocation.at(i) =
            std::min(std::max(0.0, demandMbps.at(i)), std::max(0.0, weights.at(i)) * lambda);
    }
...
void
RuDuCuAllocationTick(Ptr<OutputStreamWrapper> traceFile,
                     uint32_t ueCount,
                     double baseDemandPerUeMbps,
                     double ruCapacityMbps,
                     double duCapacityMbps,
                     double duCostPerMbps,
                     std::vector<double> cuWeights,
                     Time controlPeriod)
{
    std::vector<double> demand(ueCount, 0.0);
    const double nowS = Simulator::Now().GetSeconds();

    for (uint32_t i = 0; i < ueCount; ++i)
    {
        const double wave = 0.65 + 0.35 * std::sin(0.7 * nowS + 0.5 * static_cast<double>(i));
        demand.at(i) = std::max(0.05 * baseDemandPerUeMbps, baseDemandPerUeMbps * wave);
    }

    const std::vector<double> allocation = ComputeWeightedPfAllocation(
        demand,
        cuWeights,
        ruCapacityMbps,
        duCapacityMbps,
    ...
    const double totalDemand =
        std::accumulate(demand.begin(), demand.end(), 0.0, [](double acc, double d) {
            return acc + d;
        });
    const double totalAllocation =
        std::accumulate(allocation.begin(), allocation.end(), 0.0, [](double acc, double a) {
            return acc + a;
        });

    std::stringstream perUeAlloc;
    for (uint32_t i = 0; i < allocation.size(); ++i)
    {
        perUeAlloc << allocation.at(i);
    ...
    *traceFile->GetStream() << nowS << ',' << totalDemand << ',' << totalAllocation << ','
                            << ruCapacityMbps << ',' << duCapacityMbps << ',' << duCostPerMbps
                            << ',' << '"' << perUeAlloc.str() << '"' << std::endl;

    Simulator::Schedule(controlPeriod,
                        &RuDuCuAllocationTick,
                        traceFile,
                        ueCount,
                        baseDemandPerUeMbps,
                        ruCapacityMbps,
...
main(int argc, char* argv[])
{
    ...
    bool enableRuDuCuController = true;
    Time ruDuCuControlPeriod = MilliSeconds(50);
    double ruCapacityMbps = 120.0;
    double duCapacityMbps = 90.0;
    double duCostPerMbps = 1.0;
    std::string cuSliceWeights = "1.0,1.0";
    ...
    cmd.AddValue("burstGeneratorType",
                 ...
                 burstGeneratorType);
    cmd.AddValue("enableRuDuCuController",
                 "Enable periodic RU/DU/CU control-plane allocation emulation",
                 enableRuDuCuController);
    cmd.AddValue("ruDuCuControlPeriod",
                 "Control loop period for RU/DU/CU allocation emulation",
                 ruDuCuControlPeriod);
    cmd.AddValue("ruCapacityMbps",
                 "RU capacity budget in Mbps for allocation emulation",
                 ruCapacityMbps);
    cmd.AddValue("duCapacityMbps",
                 "DU processing budget in Mbps for allocation emulation",
                 duCapacityMbps);
    cmd.AddValue("duCostPerMbps",
                 "DU processing cost multiplier per allocated Mbps",
                 duCostPerMbps);
    cmd.AddValue("cuSliceWeights",
                 "Comma-separated CU slice weights, repeated across UEs",
                 cuSliceWeights);
    ...
    if (enableRuDuCuController)
    {
        Ptr<OutputStreamWrapper> ruDuCuTrace = ascii.CreateFileStream("ruducu-allocation.csv");
        *ruDuCuTrace->GetStream()
            << "Time_s,TotalDemand_Mbps,TotalAllocation_Mbps,RuCap_Mbps,DuCap_Mbps,"
            << "DuCostPerMbps,PerUeAllocation_Mbps" << std::endl;

        const double baseDemandPerUeMbps = ToMbpsFromDataRateString(appRate);
        std::vector<double> cuWeights = ParseWeightsCsv(cuSliceWeights, ueLowLatContainer.GetN());

        Simulator::Schedule(Seconds(0.1),
                            &RuDuCuAllocationTick,
                            ruDuCuTrace,
                            ueLowLatContainer.GetN(),
                            baseDemandPerUeMbps,
                            ruCapacityMbps,
                            duCapacityMbps,
                            duCostPerMbps,
