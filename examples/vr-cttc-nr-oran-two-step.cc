...
main(int argc, char* argv[])
{
    ...
    bool enableStepLoad = false;
    Time stepLoadTime = Seconds(5.0);
    double stepLoadFraction = 0.5;
    Time earlyStartMin = Seconds(2.5);
    Time earlyStartMax = Seconds(3.0);
    ...
    cmd.AddValue("enableStepLoad",
                 "if true, starts a fraction of UEs later to create a sudden load increase",
                 enableStepLoad);
    cmd.AddValue("stepLoadTime",
                 "late cohort start time used when enableStepLoad=true",
                 stepLoadTime);
    cmd.AddValue("stepLoadFraction",
                 "fraction [0,1] of UEs that start at stepLoadTime when enableStepLoad=true",
                 stepLoadFraction);
    cmd.AddValue("earlyStartMin",
                 "minimum randomized start time for the early UE cohort",
                 earlyStartMin);
    cmd.AddValue("earlyStartMax",
                 "maximum randomized start time for the early UE cohort",
                 earlyStartMax);
    ...
    NS_ABORT_IF(stepLoadFraction < 0.0 || stepLoadFraction > 1.0);
    NS_ABORT_IF(earlyStartMin > earlyStartMax);
    ...
    Ptr<UniformRandomVariable> randomStart =
        CreateObjectWithAttributes<UniformRandomVariable>("Min",
                                                          DoubleValue(earlyStartMin.GetSeconds()),
                                                          "Max",
                                                          DoubleValue(earlyStartMax.GetSeconds()));

    uint32_t lateStartUeCount = 0;
    if (enableStepLoad)
    {
        lateStartUeCount = static_cast<uint32_t>(stepLoadFraction *
                                                 static_cast<double>(ueLowLatContainer.GetN()) +
                                                 0.5);
    }
    uint32_t firstLateUeIdx = (lateStartUeCount >= ueLowLatContainer.GetN())
                                  ? 0
                                  : (ueLowLatContainer.GetN() - lateStartUeCount);
    ...
    *txFragmentTrace->GetStream()
    ...
    {
        bool isLateUe = enableStepLoad && (i >= firstLateUeIdx);
        Time startTime = isLateUe ? stepLoadTime : Seconds(randomStart->GetValue());
